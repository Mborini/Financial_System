...
export async function GET() {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return new Response(JSON.stringify({ message: "No deductions found" }), { status: 404 });
    }

    return new Response(JSON.stringify(result.rows), { status: 200 });
  } catch (error) {
    return new Response(JSON.stringify({ error: "Error fetching deductions" }), { status: 500 });
  } finally {
    client.release();
  }
}
export async function POST(request) {
    ...
    try {
      ...
      return new Response(JSON.stringify({ message: "Deduction added and salary account updated" }), { status: 201 });
    } catch (error) {
      await client.query("ROLLBACK");
      console.error("Error adding deduction and updating salary account:", error);
      return new Response(JSON.stringify({ error: "Error adding deduction and updating salary account" }), { status: 500 });
    } finally {
      client.release();
    }
  }
  ...
  export async function PUT(request) {
    const { id, amount, date, employee_id, old_amount, deduction_type } = await request.json(); // No salary_period
    const client = await connectToDatabase();
    ...
    try {
      ...
      await client.query("COMMIT");
      return new Response(JSON.stringify({ message: "Deduction and salary account updated" }), { status: 200 });
    } catch (error) {
      await client.query("ROLLBACK");
      console.error("Error updating deduction and salary account:", error);
      return new Response(JSON.stringify({ error: "Error updating deduction and salary account" }), { status: 500 });
    } finally {
      client.release();
    }
  }
  ...
  export async function DELETE(request) {
    const { id, employee_id, amount } = await request.json(); // No salary_period
    const client = await connectToDatabase();
    ...
    try {
      ...
      await client.query("DELETE FROM Deductions WHERE id = $1 AND employee_id = $2", [id, employee_id]);
      ...
      await client.query("COMMIT");
      return new Response(JSON.stringify({ message: "Deduction deleted and salary account updated" }), { status: 200 });
    } catch (error) {
      await client.query("ROLLBACK");
      console.error("Error deleting deduction and updating salary account:", error);
      return new Response(JSON.stringify({ error: "Error deleting deduction and updating salary account" }), { status: 500 });
    } finally {
      client.release();
    }
  }
